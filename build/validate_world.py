...
def main():
    ...
    check(t.get("ForceGameType").py_int == 1, "ForceGameType=1（既存プレイヤーもアドベンチャーへ寄せる）")
    abilities = t.get("abilities", {})
    check(abilities.get("instabuild").py_int == 0, "instabuild=0（クリエイティブ能力を無効化）")
    check(abilities.get("mayfly").py_int == 0, "mayfly=0（飛行能力を無効化）")
    check(abilities.get("build").py_int == 0, "build=0（建築能力を無効化）")
    check(abilities.get("mine").py_int == 0, "mine=0（破壊能力を無効化）")
    check(abilities.get("doorsandswitches").py_int == 1, "doorsandswitches=1（ボタン操作は許可）")
    check(t.get("hasBeenLoadedInCreative").py_int == 0, "hasBeenLoadedInCreative=0")
