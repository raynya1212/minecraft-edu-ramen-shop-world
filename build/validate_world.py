...
def main():
    ...
    check(t.get("ForceGameType").py_int == 0, "ForceGameType=0（MEEでボタン操作を妨げない）")
    abilities = t.get("abilities", {})
    check(abilities.get("instabuild").py_int == 1, "instabuild=1（MEEでボタン操作を妨げない）")
    check(abilities.get("mayfly").py_int == 1, "mayfly=1（旧来の操作互換）")
    check(abilities.get("build").py_int == 1, "build=1（旧来の操作互換）")
    check(abilities.get("mine").py_int == 1, "mine=1（旧来の操作互換）")
    check(abilities.get("doorsandswitches").py_int == 1, "doorsandswitches=1（ボタン操作は許可）")
    check(t.get("hasBeenLoadedInCreative").py_int == 1, "hasBeenLoadedInCreative=1（旧来の操作互換）")
